  ...
  try {
    ...
    for (let i = 0; i < mintPassHolders.length; i++) {
      ...
      try {
        ...
        let foundRecordWallet =
          ensData[ensRecordWalletIndex]?.[
            "Wallet Address that the NFT was minted"
          ];

        let foundRecordAddress = ensData[ensRecordAddressIndex]?.address;
        ...
        if (mintPassAllowlistData.includes(address)) {
          ...
          c = c + 1;
          if (kycRecord?.address || ensRecord?.address) {
            ...
            if (existingProfileIndex === -1) {
              ...
              await certificate.cert_profiles.push({
                ...
                kyc_verification:
                  kycRecord?.Verified === "Yes" ||
                  kycRecord?.Verified === "DONE"
                    ? "Completed"
                ...
                ens_verification:
                  ensRecord?.Verified === "Yes" ||
                  ensRecord?.Verified === "DONE"
                    ? "Completed"
